 ...
 */
Coordinates
ParseCoordinates(std::string_view str)
{
...
void InputReader::ApplyCommands([[maybe_unused]] TransportCatalogue &catalogue) const
{
    ...
    for (const CommandDescription &command : commands_)
    {
        if (command.command == "Stop"s)
        {
            ...
            command.command == "Bus"s)
        {
            routes.push_back(std::move(command));
        }
    }
    for (const CommandDescription &stop : stops)
    {
        catalogue.AddStop(move(stop.id), std::move(ParseCoordinates(stop.description)));
    }
